...
for epoch in range(0, 2_500):
      ...
      if epoch%1 ==0: 
        ...
        full_epi_rewards = np.stack([t.numpy()[0][0] for t in full_epi_rewards])
        acc_reward+= np.sum(full_epi_rewards)
        
        full_epi_rewardsqos = np.stack([t.numpy()[0][0] for t in full_epi_rewardsqos])
        acc_reward_qos+= np.sum(full_epi_rewardsqos)
        
        full_epi_rewardsbh = np.stack([t.numpy()[0][0] for t in full_epi_rewardsbh])
        acc_reward_bh+= np.sum(full_epi_rewardsbh)
        
        full_epi_rewardsuep = np.stack([t.numpy()[0][0] for t in full_epi_rewardsuep])
