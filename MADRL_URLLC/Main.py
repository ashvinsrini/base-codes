...
if __name__ == "__main__":
    ...
    actor_target2 = Actor(state_dim, action_dim)
    critic_target2 = Critic(state_dim, action_dim)

    # Copy initial weights to target networks
    actor_target2.load_state_dict(actor2.state_dict())
    ...
    optimizer_actor2 = optim.Adam(actor2.parameters(), lr=LR_ACTOR)
    optimizer_critic2 = optim.Adam(critic2.parameters(), lr=LR_CRITIC)
    ...
    for epi in range(num_episodes):
        ...
        for t in tqdm(range(time_steps)):
            ...
            weighted_reward2, _ , _, reward_res2 = environ.get_rewards(SNR, action)
            total_reward2 += weighted_reward2
            ...
            if len(replay_buffer) >= BATCH_SIZE:
                    ...
                    values2 = critic2(states, actions)
                    loss_critic = nn.MSELoss()(values, targets)
                    loss_critic2 = nn.MSELoss()(values2, targets)
                    ...
                    loss_actor2 = -critic2(states, actor2(states)).mean()
                    optimizer_actor2.zero_grad()
                    loss_actor2.backward()
                    optimizer_actor2.step()
                    ...
                    for param, target_param in zip(critic2.parameters(), critic_target2.parameters()):
                        target_param.data.copy_(TAU * param.data + (1 - TAU) * target_param.data)
                    for param, target_param in zip(actor2.parameters(), actor_target2.parameters()):
                        target_param.data.copy_(TAU * param.data + (1 - TAU) * target_param.data)
        ...
        print('epoch',epi,'rewards_agent1', total_reward/(t*time_steps),
              'rewards_agent2', total_reward2/(t*time_steps))
